...
def create_graph(df):
    ...
    columns = df.columns.drop('amount')
    df[columns] = df[columns].divide(df[columns].iloc[0] / df['amount'].iloc[0])
    df.plot()
    plt.show()
...
def render(
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            data_render.append(env.render_data)

        create_graph(pd.DataFrame(data_render))
